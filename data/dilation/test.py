...
def compute_mean(im_file):
    with open(im_file) as f:
        imageList = f.readlines()
    sample = cv2.imread(imageList[0].strip('\n'))
    height, width, channels = sample.shape
    mean_image = mean_images = np.zeros((height,width,channels), dtype=float)

    for im_id in range(len(imageList)):
        im = cv2.imread(imageList[im_id].strip('\n'))
        mean_image = mean_image + im

    mean_image = mean_image/len(imageList)
    b =  mean_image[:,:,0].sum()/(height*width)
    g =  mean_image[:,:,1].sum()/(height*width)
    r =  mean_image[:,:,2].sum()/(height*width)
    return [b,g,r]
...
def test_image(options):
    ...
    for i in range(len(image_names)):
        ...
        print('Predicting', image_paths[i])
...
def process_options(options):
    ...
    if options.mean is None:
        options.mean = compute_mean(options.image_list)        
...
def main():
    ...
    parser.add_argument('--mean', nargs='*',
                        help='I\'ll give you a function which can automatically calculate means of image list'
                        'Mean pixel value (BGR) for the dataset.\n'
